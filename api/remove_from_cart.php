...
file_put_contents($logFile, date('Y-m-d H:i:s') . " - Démarrage de la requête remove_from_cart\n", FILE_APPEND);
...
file_put_contents($logFile, date('Y-m-d H:i:s') . " - Requête suppression: " . $delete_query . "\n", FILE_APPEND);
...
if ($conn->query($delete_query)) {
    ...
    file_put_contents($logFile, date('Y-m-d H:i:s') . " - Succès: Article supprimé du panier\n", FILE_APPEND);
} else {
    http_response_code(500);
    echo json_encode([
        "success" => false,
    ...
    file_put_contents($logFile, date('Y-m-d H:i:s') . " - Erreur lors de la suppression: " . $conn->error . "\n", FILE_APPEND);
}
